...
static void EscreverArvore(NoArvore* no, ofstream& arq, char* byte, int* bits)
{
	...
	{
		*byte <<= 1;
		*byte |= 0; //grava 0 no byte
		(*bits)++;
	}

	if (*bits == 8)
	{
		arq.write(byte, 1); //escreve o byte quando está completo
	}
...
static void Compactar()
{
	...
	if (abriuArquivo) //
	{
		...
		ofstream arq("C:\\Users\\Hugo\\Documents\\Cotuca\\TPOO\\compactador_huffman/compactado.dat", ios::binary);
	...
	ifstream teste("C:\\Users\\Hugo\\Documents\\Cotuca\\TPOO\\compactador_huffman/compactado.dat", ios::binary);
